...
const app = express();
app.use(compression());
// Serve template CSS files with long-lived browser cache (filenames bumped on change)
...
// ─── PERSISTENT DEMO CACHE (disk) ─────────────────────────────────────────────
// Demos generated by Anthropic + Google Places are expensive (~3-7s).
// Persist rendered HTML to disk so cache survives Railway in-instance restarts.
// NOTE: For survival across full Railway redeploys, this directory should be
// backed by a Railway volume. Without a volume, this is best-effort.
const CACHE_DIR = path.join(__dirname, 'cache');
try { fs.mkdirSync(CACHE_DIR, { recursive: true }); } catch (err) {
  console.error('Could not create cache dir:', err.message);
}
function diskCachePath(cacheKey) {
  // cacheKey looks like "place_id:layout" — sanitize for filesystem
  return path.join(CACHE_DIR, cacheKey.replace(/[^a-zA-Z0-9._-]/g, '_') + '.html');
}
function diskCacheGet(cacheKey) {
  try { return fs.readFileSync(diskCachePath(cacheKey), 'utf8'); }
  catch { return null; }
}
function diskCacheSet(cacheKey, html) {
  try { fs.writeFileSync(diskCachePath(cacheKey), html, 'utf8'); }
  catch (err) { console.error('Cache write failed for', cacheKey, '-', err.message); }
}
function diskCacheDelete(cacheKey) {
  try { fs.unlinkSync(diskCachePath(cacheKey)); } catch {}
}
function diskCacheHas(cacheKey) {
  try { return fs.existsSync(diskCachePath(cacheKey)); } catch { return false; }
}
...
app.get('/demo-status', (req, res) => {
  ...
  // Disk-backed cache survives in-instance restarts. Warm memory on hit.
  if (diskCacheHas(cacheKey)) {
    const html = diskCacheGet(cacheKey);
...
app.get('/demo', async (req, res) => {
  ...
  // Cache hit (memory) — serve immediately, skip loading page
  ...
  // Cache hit (disk) — warm memory and serve. Survives in-instance restart.
  if (refresh !== 'true' && diskCacheHas(cacheKey)) {
    const fromDisk = diskCacheGet(cacheKey);
    if (fromDisk) {
      demoCache.set(cacheKey, fromDisk);
      console.log(`💾 Disk hit: ${cacheKey}`);
      res.setHeader('Content-Type', 'text/html');
      return res.send(fromDisk);
    }
  }

  // On explicit refresh, clear stale error + memory + disk so we retry
  if (refresh === 'true') {
    ...
    (async () => {
      try {
        ...
        if (industry === 'unsupported') {
          const placeholder = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Coming Soon — HelloSite</title></head><body style="font-family:sans-serif;background:#FFF7E8;color:#17324D;display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center;padding:2rem;margin:0;"><main><h1 style="font-size:1.75rem;margin-bottom:.75rem;">Coming Soon</h1><p style="opacity:.7;max-width:360px;margin:0 auto 1.5rem;line-height:1.7;">We currently support trades, grooming, wellness, pet care, and retail.</p><a href="https://gethellosite.com" style="background:#17324D;color:#fff;padding:.75rem 1.5rem;border-radius:100px;text-decoration:none;font-weight:600;display:inline-block;">Learn More</a></main></body></html>`;
          demoCache.set(cacheKey, placeholder);
          diskCacheSet(cacheKey, placeholder);
        ...
        if (reviewCount < 10) {
          const placeholder = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Not Quite Ready — HelloSite</title></head><body style="font-family:sans-serif;background:#FFF7E8;color:#17324D;display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center;padding:2rem;margin:0;"><main><h1 style="font-size:1.75rem;margin-bottom:.75rem;">Not Quite Ready</h1><p style="opacity:.7;max-width:400px;margin:0 auto 1.5rem;line-height:1.7;">This business needs at least 10 Google reviews before we can build a great demo.</p><a href="https://gethellosite.com" style="background:#17324D;color:#fff;padding:.75rem 1.5rem;border-radius:100px;text-decoration:none;font-weight:600;display:inline-block;">Learn More</a></main></body></html>`;
          demoCache.set(cacheKey, placeholder);
          diskCacheSet(cacheKey, placeholder);
          demoProgress.delete(cacheKey);
          return;
