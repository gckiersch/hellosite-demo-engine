...
IMPORTANT: Never use em dashes (—) in any copy. Use periods or commas instead.
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  @media(max-width:480px){h1{font-size:clamp(1.5rem,7vw,2.2rem)!important;}}
</style>
</head>
<body>${body}</body>
</html>`;
...
function reviewsHTML(reviews, rating, reviewCount, primary, theme) {
  ...
  return `<section id="reviews" style="background:${bg};padding:5rem 4rem;" class="mob-pad">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1.25rem;" class="mob-stack">
      ${reviews.map(r => {
        const fullText = (r.text?.text||'');
        let txt = fullText;
        let truncated = false;
        if (fullText.length > 250) {
          txt = fullText.substring(0, 250);
        ...
        return `<div style="background:${bg2};border:1px solid ${border};padding:1.5rem;border-radius:4px;">
          ...
          <p style="font-size:.82rem;color:${muted};line-height:1.7;margin-bottom:1rem;font-style:italic;">"${txt}${truncated?'…':''}"</p>
  ...
  return baseHTML(name, theme, `
    ...
    <section style="min-height:100vh;position:relative;display:flex;align-items:center;overflow:hidden;">
      ...
      <div style="position:relative;z-index:2;padding:9rem 5rem 6rem;max-width:720px;" class="mob-pad fu">
        ...
        <h1 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(2rem,9vw,8rem);line-height:.9;letter-spacing:.01em;margin-bottom:1.5rem;color:#f5f2ed;" class="fu d1">${formatHeadline(copy.hero_headline, p)}</h1>
...
function layoutSplit(place, copy, photos, industry) {
  ...
  const city = address.split(',')[1]?.trim() || 'Local';
  ...
  return baseHTML(name, theme, `
    ...
    <section style="min-height:100vh;display:grid;grid-template-columns:1fr 1fr;" class="mob-stack">
      <div style="display:flex;flex-direction:column;justify-content:center;padding:9rem 4rem 5rem 5rem;background:${panelBg};position:relative;z-index:2;" class="mob-pad">
        ...
        <h1 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(2rem,6vw,6.5rem);line-height:.88;letter-spacing:.02em;margin-bottom:1.5rem;color:${textColor};" class="fu d1">${formatHeadline(copy.hero_headline, a)}</h1>
        ...
        <div style="display:grid;grid-template-columns:repeat(3,auto);gap:2.5rem;margin-top:3.5rem;padding-top:2.5rem;border-top:1px solid ${isDark?'rgba(255,255,255,.06)':'rgba(0,0,0,.06)'};" class="fu d4">
          ...
          <div style="min-width:0;"><div style="font-family:'Bebas Neue',sans-serif;font-size:1.75rem;color:${a};line-height:1.2;word-break:break-word;hyphens:auto;">${city}</div><div style="font-size:.6rem;color:${mutedColor};letter-spacing:.1em;text-transform:uppercase;margin-top:.2rem;">Local</div></div>
        </div>
      </div>
  ...
  return baseHTML(name, theme, `
    ...
    <section style="min-height:100vh;position:relative;display:flex;align-items:flex-end;overflow:hidden;">
      ...
      <div style="position:relative;z-index:2;padding:0 5rem 5rem;width:100%;max-width:800px;" class="mob-pad fu">
        ...
        <h1 style="font-family:'Playfair Display',Georgia,serif;font-size:clamp(2rem,7vw,6rem);font-weight:700;line-height:1.0;letter-spacing:-.02em;color:#fff;margin-bottom:1.25rem;" class="fu d1">${copy.hero_headline.replace(/\\n|\n/g,'<br>')}</h1>
